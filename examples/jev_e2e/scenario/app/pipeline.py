...
The handler is called as ``handler(request, ctx)`` with a fresh
``Context`` for each request. Middlewares do not receive the context yet;
they share per-request data through ``Request.extras``.
"""
...
class Request:
    ...
    # Scratch space middlewares share for one request.
