...
class Recorder(Middleware):
    ...
    def process_request(self, request):
        ...
        request.extras.setdefault("seen", []).append(self.name)
...
class PipelineTest(unittest.TestCase):
    ...
    def test_extras_are_shared_between_middlewares(self):
        log = []
        request = Request("GET", "/")
        Pipeline(ok_handler, [Recorder("a", log), Recorder("b", log)]).dispatch(request)
        self.assertEqual(request.extras["seen"], ["a", "b"])
