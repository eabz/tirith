...
5. Do not implement the same functionality twice. If a task turns out to be covered by code that already exists, verify its acceptance criteria against that code and say so in your summary.
...
1. Implement the task in `RUN_DIR/repo` and add tests. Run `python3 -m unittest` until it passes. The task's `paths` are a hint, not an order.
2. If you changed something other code depends on (a signature, a rename, behavior other middlewares rely on), publish it: `tb notice_publish '{"kind":"signature","summary":"...","affected_paths":["..."]}'`.
3. End your turn with a short summary of how each acceptance criterion is met.
...
When you are asked for your final report, report briefly: the tasks you completed (id and title), the files you changed, the final `python3 -m unittest` result, the notices and messages you sent, every edit that was refused and how you resolved it, and anything left unfinished.
